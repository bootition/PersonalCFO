...
6. **UI/交互类阶段额外必过项（P7.7 复盘新增，防止"功能能跑但体验返工"）**：
   - **参照物对比**：逐一对照 `docs/decisions/02_Paisa到Wealthfolio页面映射.md` 的映射表与验收标准，页面对不上即 FAIL；
   - **用户旅程走查**：用真实账本数据按"新用户首装 → 上传账单 → ①②③④ → 同步进首页 → 遍历侧栏每个可见页"完整走一遍（不允许只用 mock/合成数据）；
   - **真实数据回归**：跑 `python scripts/e2e-sweep.py --base-url http://localhost:7500`，失败项必须清零（空白页/英文残留/图表 19xx/首页交易明细/隐藏路由崩页/默认主题 不在列，任一命中即 FAIL）；
   - **移动端**：375px 宽抽查导航开合与内容溢出。
...
- 账单样本在项目外（`D:\<本机用户名>\<本地目录名>\消费记录`），含敏感财务信息，只做本地分析，不得写入 git 或入库文档
