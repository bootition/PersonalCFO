...
| P0 仓库与环境 | ✅ 完成：主仓干净并与 origin/main 同步；.gitignore 抽查全部命中；venv 重建（pypdf 6.17.0 + pdfplumber 0.11.10）；`finance.py check` exit 0 | `docs/dev/任务计划.md` 执行日志 2026-09-06 |
...
1. **FIXME 残余（P7.21 批量定性后）**：支付宝 **金额占比 0.61% / 279 笔**、微信 **金额占比 10.88% / 36 笔**。已按用户确认把课程分期→书籍学习、固定充值→日常三餐、家人转入→`Income:生活费`；**用户明确保留**亲属账户互转（对方账户不确定）与其余小额（红包/AA/杂项）为 FIXME，后续可在界面逐笔过。逐笔明细只在本地 `docs/本地待办_DO_NOT_COMMIT.md`（不入库）。
...
7. **花呗拆分残余（P7.14 后）**：脚本已自动分类——退款已入账 4/亲情卡·代付 11/期外还款 1 不再误报；剩余 **PDF 侧 6 笔（合计 17 笔小额）+ 账单侧 5 笔（4 笔外卖平台小额）**。用户决定：因支付宝账单截止日早于花呗账单，先不手工处理，待导入最新账单后自然匹配（清单在本地 `reports/huabei-split-*.txt`）。
...
| `docs/dev/任务计划.md`（根目录） | 分阶段实施计划与执行日志 | 每完成一条勾一条 |
