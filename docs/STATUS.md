...
| **P7.21 用户定性批量落地 + 投资/银行接入手册**（2026-09-12） | ✅ ①**FIXME 批量定性**（用户弹窗确认）：`单挑啊`课程首付/分期 12 笔 → `Expenses:书籍学习`；`充值-普通充值` 4 笔 → `Expenses:日常三餐`；`邱景衡`收入 8 笔 → 新科目 `Income:生活费`；规则追加至 `config/alipay.yaml`/`wechat.yaml` 末尾（后命中覆盖）。重导后支付宝 FIXME **4.06% → 0.61%**（279 笔 / 2,934.43 元）、微信 36 笔 / 6,908.66 元（10.88%）；用户明确**保留**邱鸿账户互转（对方账户不确定）与其余小额 FIXME。`check OK`、`reconcile-check` 全绿（含跨源 9 科目 0 不一致）。②`sync_fair_value.py --write` 现自动刷新 Paisa include（防漏加导致口径分裂）；合成 holdings CSV 干跑验证映射（含未映射账户警告）。③新增 `docs/runbooks/04_Wealthfolio与银行接入.md`：Wealthfolio 账户名对照表（哈利布朗/攒股收息/虚拟货币/CS饰品）、录入与季度回写步骤；deg 已支持的银行 provider 清单、银行 CSV 接入流程（等用户导出文件后写配置）与半追踪期对账办法 | main 待提交（本地） |
...
1. **FIXME 残余（P7.21 批量定性后）**：支付宝 **0.61% 金额 / 279 笔（2,934.43 元）**、微信 **10.88% / 36 笔（6,908.66 元）**。已按用户确认把课程分期→书籍学习、固定充值→日常三餐、邱景衡转入→`Income:生活费`；**用户明确保留**邱鸿账户互转（对方账户不确定）与其余小额（红包/AA/杂项）为 FIXME，后续可在界面逐笔过。逐笔明细只在本地 `docs/本地待办_DO_NOT_COMMIT.md`（不入库）。
