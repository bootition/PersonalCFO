...
| **P7.10 资产线重做（净值页完成，P2-4 部分）**（2026-09-12） | ✅ 净值页对齐 Wealthfolio net-worth：大数字 hero（净资产 2.2rem）+ 次级指标卡（净投资/盈亏/XIRR）+ 走势图卡片（标题+图例，高 420）；移除 Paisa 旧 LevelItem/BoxLabel 行。实测 hero 完整、图表 741 path、2 卡片；e2e-sweep 0 失败。**P2-4 仍进行中**：投资/收益页待按同款结构重做 | fork 待提交（本地） |
