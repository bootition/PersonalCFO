...
4. **历史微信 journal 是修复前导入的，缺第一条数据**：需重新上传同一微信账单触发覆盖重导（`finance.py import` 写同名 journal）后才完全修正；不重导则仅缺该 1 条（金额小）。
