...
| **P7.12 财报页卡片化（P2-5 完成）**（2026-09-12） | ✅ ①利润表：顶部 4 张数值卡（收入/支出/税务·利息/净利润带色，按所选财年聚合），期初·期末·变动条与图表改 `.pcfo-card`，科目矩阵表容器卡片化；②资产余额页：4 张数值卡（持仓市值/累计投入/累计转出/总盈亏）+ 表格卡片；③负债余额页：4 张数值卡（负债余额带色/累计提取/累计已还/累计利息）+ 表格卡片，空状态保留。实测三页各 4 卡、无报错；e2e-sweep 31 路由 0 失败 | fork 待提交（本地） |
